...
public class SessionMapper implements RowMapper<SessionEntity> {
    ...
    public SessionEntity mapRow(ResultSet resultSet, int i) throws SQLException {
        return new SessionEntity(resultSet.getString(SessionEntity.LOGIN), resultSet.getString(SessionEntity.TIME_OF_BEGIN), resultSet.getString(SessionEntity.TIME_RECENT_ACTIVITY));
    }
}